...
import { Zap, LayoutDashboard, Code2, History, LogOut, Menu, X, User, ChevronDown } from 'lucide-react';
    ...
    <header className="sticky top-0 z-50" style={{ background: 'rgba(2,4,8,0.85)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
      <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-16">
        ...
        <Link to="/dashboard" className="flex items-center gap-2">
          ...
          <span className="font-display font-bold text-base text-white">Code<span className="gradient-text">Lens</span></span>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-3">
          <span className="text-xs px-3 py-1.5 rounded-lg" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: '#475569' }}>
          ...
          <div className="relative">
            <button onClick={() => setUOpen(!uOpen)}
              ...
              <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: 'rgba(6,182,212,0.15)', border: '1px solid rgba(6,182,212,0.2)' }}>
            ...
            {uOpen && (
              <div className="absolute right-0 mt-2 w-52 card py-1 shadow-2xl" style={{ boxShadow: '0 20px 60px rgba(0,0,0,0.6)' }}>
          ...
          {links.map(({ to, label, icon: Icon }) => (
            <Link key={to} to={to} onClick={() => setOpen(false)}
              className={`flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-medium transition-all
                ${pathname === to ? 'text-cyan-400 bg-cyan-500/08' : 'text-slate-400 hover:text-slate-200'}`}>
              <Icon className="w-4 h-4" />{label}
            </Link>
          ))}
          <button onClick={handleLogout}
